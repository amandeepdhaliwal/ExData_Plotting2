...
# form plot data
plot_data <- NEI %>% left_join(SCC, by = "SCC") %>% filter(fips == 24510) %>% filter(EI.Sector %in% veh_sectors) %>% group_by(year) %>% summarise(totalPM25 = sum(Emissions))

# render plot
png("plot5.png", height = 480, width = 480)
ggplot(plot_data, aes(year, totalPM25)) + geom_line() + ggtitle("Total PM2.5 emissions from Motor Vehicles in Baltimore City")
dev.off()